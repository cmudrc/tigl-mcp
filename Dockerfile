...
# Install TiGL + geometry deps in Python 3.12 env, and SU2 in a separate env.
RUN mamba create -n tigl -y -c conda-forge python=3.12 meshio numpy gmsh python-gmsh \
 && mamba install -n tigl -y -c dlr-sc -c conda-forge dlr-sc::tigl3 dlr-sc::tixi3 pythonocc-core \
 && mamba create -n su2 -y -c conda-forge python=3.11 su2 \
 && mamba clean -afy

ENV PATH=/opt/conda/envs/tigl/bin:/opt/conda/envs/su2/bin:$PATH
